...
function regenerateOverlays() {
    ...
    for (let i = 1; i <= totalImages; i++) {
        showImage(i); // Reload images with new overlays
    }
}
...
function showImage(index) {
    ...
    img.src = `https://bafybeienhehurqglteumnuzybndsk5slupakcfb3ya2x26saamug3bffji.ipfs.flk-ipfs.xyz/images/${index}.png`;
    ...
    overlayPaths.forEach((path, i) => {
        ...
        overlay.className = `overlay overlay${i + 1}`;
        const overlayFileName = assignedOverlays[path];
        const overlayUrl = `https://bafybeienhehurqglteumnuzybndsk5slupakcfb3ya2x26saamug3bffji.ipfs.flk-ipfs.xyz/Overlays/${path.charAt(0).toUpperCase() + path.slice(1)}/${sanitizeFileName(overlayFileName)}`;
        overlay.style.backgroundImage = `url(${overlayUrl})`;
        imageDiv.appendChild(overlay);
        overlayElements.push(overlay);

        overlay.addEventListener('click', () => {
            overlay.style.transform = overlay.style.transform === 'scale(1.5)' ? 'scale(1)' : 'scale(1.5)';
        });

        let randomEffect;
        do {
            randomEffect = effects[Math.floor(Math.random() * effects.length)];
        } while (usedEffects.has(randomEffect));
        usedEffects.add(randomEffect);
        overlay.classList.add(randomEffect);
    });
    ...
    if (overlayElements.length > 0) {
        ...
        overlayPaths.forEach((path, i) => {
            ...
            button.addEventListener('click', () => {
                overlayElements[i].style.display = overlayElements[i].style.display === 'none' ? 'block' : 'none';
            });
        ...
        imageDiv.addEventListener('click', () => {
            buttonsDiv.style.visibility = buttonsDiv.style.visibility === 'hidden' ? 'visible' : 'hidden';
        });
    }
...
// Load assigned overlays on page load
loadAssignedOverlays();
for (let i = 1; i <= totalImages; i++) {
    showImage(i);
}

// Regenerate overlays from the console
